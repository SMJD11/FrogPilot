...
class Parser:
  ...
  def parse_outputs_v7(self, outs: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    """Single-pass parser for V7 models (mirrors April12th).

    V7 models produce a single combined output, so each key must be processed
    exactly once. The split parse_vision_outputs/parse_policy_outputs approach
    double-processes shared keys like lane_lines, causing reshape failures.
    """
